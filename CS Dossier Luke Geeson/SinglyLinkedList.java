...
public class SinglyLinkedList implements Serializable
{
    ...
    public void addRecord(Node newData, Node trailNode)
    {
        ...
        else if (newData.getData().getSurname().equalsIgnoreCase(trailNode.getData().getSurname()))
        {                                                   //if the surnames are identical - store new one after old
            newData.setNext(trailNode.getNext());           //set new data.next to trail node.next
            trailNode.setNext(newData);                     //set trailnode.next as new data
        }
    ...
    public static Node changeNode(Node nodeToChange, String changeDecision, String changeInput)
    {
        ...
        else if (changeDecision.equalsIgnoreCase("SURNAME") || changeDecision.equalsIgnoreCase("SECONDNAME") || changeDecision.equalsIgnoreCase("SECOND NAME") || changeDecision.equalsIgnoreCase("S"))
    ...
    public void sortList()
    {
        ...
        else if (getHead().getNext() == null)               //if the list has one item only
        {
            System.out.println("List sorted\n");
        }
        else
        {
            Node current = getHead();                       //starts sort from the start of the  list
            boolean swapDone = true;                        //used to iterate the loop
            while (swapDone == true)
            {
                swapDone = false;                           //used to escape the loop
                while (current != null)
                {                                           //while current != last item
                    if (current.getNext() != null && current.getData().getSurname().compareTo(current.getNext().getData().getSurname()) >0)
                    {                                       //if the next item comes before the current
                        CustomerFile tempDat = current.getData();
                        current.setData(current.getNext().getData());
                        current.getNext().setData(tempDat); //swap the data
                        swapDone = true;                    //used to continue the loop
                    }
                    current = current.getNext();            //traversal through the loop
                }
                current = getHead();                        //continue at start of the loop
            }
            System.out.println("List sorted\n");
        }
    }
    ...
    public void setHead(Node input)
    {
        ...
        head = input;                                       //sets the node input as the head node
    }
    //------------------------------------------------------------------------------------------------
    /**
    ...
    public Node getLast(Node elementOfList)
    {
        ...
        {
            return elementOfList;                           //returns the last item in the list
        }
        else
        {
